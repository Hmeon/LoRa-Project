...
- [ ] Confirm updated goal statements in `README.md` and
...
- [ ] Confirm AT UART-only constraints for Waveshare SX1262 LoRa HAT.
- [ ] Confirm Air Speed preset range used in field runs (0..2) and record in
  `configs/examples/uart_record.yaml`.
- [ ] Verify that DIO1/IRQ assumptions are not present in the runtime.

## 3) Protocol and PHY
- [ ] Validate `LEN|SEQ|PAYLOAD` framing and `max_payload_bytes` enforcement.
- [ ] Ensure Air Speed preset mapping is consistent across
  `docs/phy_profiles_adr_code.md` and `configs/examples/phy_profiles.yaml`.

## 4) Data pipeline
- [ ] Confirm sensor schema order/units in `docs/sensing_pipeline.md`.
- [ ] Verify `dataset_raw.jsonl` meets training and evaluation needs.
- [ ] Define additional telemetry needed for ML prediction/optimization.

## 5) Metrics and logs
- [ ] Confirm PDR/ETX calculation rules in `docs/metrics_definition.md`.
- [ ] Define latency metrics (ACK RTT vs E2E) and required log fields.
- [ ] Decide how RSSI/SNR is captured (AT response parsing).

## 6) ML components
- [ ] Clarify the role of BAM inference vs new prediction/optimization models.
...
- [ ] Confirm licensing, security, and contribution docs.

## 8) Repo hygiene
- [ ] Review `ChirpChirp-main/` usage: keep as reference, archive, or extract.
