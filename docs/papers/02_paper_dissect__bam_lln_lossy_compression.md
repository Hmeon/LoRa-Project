...
> LLN properties (low data rate + instability) -> high packet loss -> frequent retransmission -> higher battery drain -> lower operational efficiency.
-> Therefore, "loss minimization + network performance optimization" is needed, and one tool is data compression.
...
This creates a grid:
> (compression rate) x (hidden neuron count) -> evaluate MSE/MAE surfaces
...
- Parameter selection is treated as a function of compression rate x neuron budget, and the paper hints at environment-guided selection.
...
- [ ] Describe experimental logic axes you will replicate: (compression rate x neuron budget) surfaces + reconstruction error
